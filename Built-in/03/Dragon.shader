Shader "Unlit/Dragon"
{
    ...
    SubShader
    {
            ...
            v2f vert(appdata v) 
        {
            ...
            {
				float diffuse_color = _DiffuseColor;
                float3 normal_dir = normalize(i.normalDir); 
				float3 view_dir = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz); // 方向都是从顶点出发
				...
				//透射光
                //float NdotL = saturate(dot(normal_dir, light_dir));  //Diffuse
                float back_dir = -normalize(light_dir+ normal_dir * _BasePassDistortion); // 折射
                float VdotB = saturate(dot(view_dir, back_dir));  // 用光线方向和视线方向模拟透射光
                float backlight_term = saturate(pow(VdotB, _BasePassPower)) * _BasePassScale;
                float thickness = 1.0 - tex2D(_ThicknessMap, i.uv).r;
                float3 backlight = backlight_term * _LightColor0 * thickness * _BasePassColor;
